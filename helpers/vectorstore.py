...
class VectorStore:
    def __init__(self, persist_directory: str = "./chroma_db", cache_folder: str = "./embedding_cache"):
        """
        Cross-platform vector store with multi-collection support
        """
    ...
    def close(self):
        ...
        for name, collection in self.collections.items():
            try:
                collection_path = os.path.join(self.persist_directory, name)
                self._delete_folder(collection_path)
            except Exception as e:
